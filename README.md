...
-- Started on Grading System (Grade_Calc controller, Marks model -- not committed i think so i'll have to check) 
-- Taking the templates from the Finance dept. 
-- Will add list for each Class separately, so that adding marks for subjects is easier 
-- Note : @Marwan => import grades into Excel [similar to the importing in finance] 
--
