...
class Grade9_geoT1 extends Controller
{

    public function index()
    {
        $grade1 = new Grade_9_geoT1();
        $englishTotal = isset($_SESSION['englishTotal']) ? $_SESSION['englishTotal'] : 0;
        $grade1s = $grade1->findAll();
        $this->view('grade9_geoT1', ['grade1s' => $grade1s, 'englishTotal' => $englishTotal]);
    }
    public function Term2()
    {
        $grade1 = new Grade_9_geoT2();
        $englishTotal = isset($_SESSION['englishTotal']) ? $_SESSION['englishTotal'] : 0;
        $grade1s = $grade1->findAll();
        $this->view('grade9_geoT2', ['grade1s' => $grade1s, 'englishTotal' => $englishTotal]);
    }
    public function Term3()
    {
        $grade1 = new Grade_9_geoT3();
        $englishTotal = isset($_SESSION['englishTotal']) ? $_SESSION['englishTotal'] : 0;
        $grade1s = $grade1->findAll();
        $this->view('grade9_geoT3', ['grade1s' => $grade1s, 'englishTotal' => $englishTotal]);
    }
    ...
    public function updateTableT2()
    {
        ...
        if (count($_POST) > 0) {
            ...
            if ($result) {
                // Optionally, set a success message here
                $this->redirect('Grade9_geoT1/Term2');
            } else {
                // Handle update failure
                echo "Update failed. Please try again.";
            }
    ...
    public function updateTableT3()
    {
        ...
        if (count($_POST) > 0) {
            ...
            if ($result) {
                // Optionally, set a success message here
                $this->redirect('Grade9_geoT1/Term3');
            } else {
                // Handle update failure
                echo "Update failed. Please try again.";
            }
