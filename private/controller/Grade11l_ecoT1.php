...
class Grade11l_ecoT1 extends Controller
{

    public function index()
    {
        $grade1 = new Grade_11l_ecoT1();
        $englishTotal = isset($_SESSION['englishTotal']) ? $_SESSION['englishTotal'] : 0;
        $grade1s = $grade1->findAll();
        $this->view('grade11l_ecoT1', ['grade1s' => $grade1s, 'englishTotal' => $englishTotal]);
    }
    public function Term2()
    {
        $grade1 = new Grade_11l_ecoT2();
        $englishTotal = isset($_SESSION['englishTotal']) ? $_SESSION['englishTotal'] : 0;
        $grade1s = $grade1->findAll();
        $this->view('grade11s_ecoT2', ['grade1s' => $grade1s, 'englishTotal' => $englishTotal]);
    }
    public function Term3()
    {
        $grade1 = new Grade_11l_ecoT3();
        $englishTotal = isset($_SESSION['englishTotal']) ? $_SESSION['englishTotal'] : 0;
        $grade1s = $grade1->findAll();
        $this->view('grade11l_ecoT3', ['grade1s' => $grade1s, 'englishTotal' => $englishTotal]);
    }
    ...
    public function updateTableT2()
    {
        ...
        if (count($_POST) > 0) {
            ...
            if ($result) {
                // Optionally, set a success message here
                $this->redirect('Grade11l_ecoT1/Term2');
            } else {
                // Handle update failure
                echo "Update failed. Please try again.";
            }
    ...
    public function updateTableT3()
    {
        ...
        if (count($_POST) > 0) {
            ...
            if ($result) {
                // Optionally, set a success message here
                $this->redirect('Grade11l_ecoT1/Term3');
            } else {
                // Handle update failure
                echo "Update failed. Please try again.";
            }
