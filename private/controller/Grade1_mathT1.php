...
class Grade1_mathT1 extends Controller
{

    public function index()
    {
        $grade1 = new Grade_1_mathT1();
        $englishTotal = isset($_SESSION['englishTotal']) ? $_SESSION['englishTotal'] : 0;
        $grade1s = $grade1->findAll();
        $this->view('grade1_mathT1', ['grade1s' => $grade1s, 'englishTotal' => $englishTotal]);
    }
    public function Term2()
    {
        $grade1 = new Grade_1_mathT2();
        $englishTotal = isset($_SESSION['englishTotal']) ? $_SESSION['englishTotal'] : 0;
        $grade1s = $grade1->findAll();
        $this->view('grade1_mathT2', ['grade1s' => $grade1s, 'englishTotal' => $englishTotal]);
    }
    public function Term3()
    {
        $grade1 = new Grade_1_mathT3();
        $englishTotal = isset($_SESSION['englishTotal']) ? $_SESSION['englishTotal'] : 0;
        $grade1s = $grade1->findAll();
        $this->view('grade1_mathT3', ['grade1s' => $grade1s, 'englishTotal' => $englishTotal]);
    }
    ...
    public function updateTableT2()
    {
        ...
        if (count($_POST) > 0) {
            ...
            if ($result) {
                // Optionally, set a success message here
                $this->redirect('Grade1_mathT1/Term2');
            } else {
                // Handle update failure
                echo "Update failed. Please try again.";
            }
    ...
    public function updateTableT3()
    {
        ...
        if (count($_POST) > 0) {
            ...
            if ($result) {
                // Optionally, set a success message here
                $this->redirect('Grade1_mathT1/Term3');
            } else {
                // Handle update failure
                echo "Update failed. Please try again.";
            }
